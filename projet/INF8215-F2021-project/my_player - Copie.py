    ...
    def play(self, percepts, player, step, time_left):
        ...
        def alphabeta(boardAB, depthAB, alpha, beta, maximizingPlayer):
            ...
            if maximizingPlayer:
                ...
                currentValue = simpleHeuristic(boardAB, player)
                for pmove in boardAB.get_actions(player):

                    # ici on aimerait filtrer les positions de mur considérer
                    # pour ne garder que les positions autour de l'opposant
                    # aussi, on ne considère placer un mur que si on est en désavantage
                    skip = False
                    if pmove[0] == 'WH' or pmove[0] == 'WV':
                        print("f")
                        position_op = boardAB.pawns[swap[player]]
                        if currentValue > 0 or abs(pmove[1] - position_op[0]) > 3 \
                                or abs(pmove[2] - position_op[1]) > 3 :
                            skip = True
                    # fin  filtre
                    if not skip:
                        print(depthAB, pmove)
                ...
                for pmove in boardAB.get_actions(swap[player]):
                    print(depthAB, pmove)
                    currentValue = simpleHeuristic(boardAB, swap[player])
                    ...
                    if pmove[0] == 'WH' or pmove[0] == 'WV':
                        print("f")
                        position_op = boardAB.pawns[player]
                        if currentValue > 0 or abs(pmove[1] - position_op[0]) > 3 \
                                or abs(pmove[2] - position_op[1]) > 3:
                            skip = True
                    # fin  filtre
